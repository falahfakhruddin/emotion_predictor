...
def predict_emo():
    ...
    quoted = '"' + sentence + '"'
    opening = "Your emotion is dominated by..."
    prediction = predictor(sentence)
    index = np.random.randint(0,3)
    reply = response(prediction,index)
    color_config = emotion_color(prediction)
    return render_template('index.html', sentence_text=quoted, opening_text=opening, prediction_text= prediction, reply_text = reply, color_config = color_config )
